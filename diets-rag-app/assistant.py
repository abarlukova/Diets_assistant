...
def elastic_search_text(query, diet, index_name="diets-questions"):
    search_query = {
        "size": 5,
        "query": {
            "bool": {
                "must": {
                    "multi_match": {
                        "query": query,
                        "fields": ["question^3", "answer", "diet"],
                        "type": "best_fields",
                    }
                },
                "filter": {"term": {"diet": diet}},
            }
        },
    }
...
def elastic_search_knn(field, vector, diet, index_name="diets-questions"):
    knn = {
        "field": field,
        "query_vector": vector,
        "k": 5,
...
def calculate_gpt_pricing(prompt_tokens, completion_tokens, 
                                 price_per_1m_prompt_tokens, 
                                 price_per_1m_completion_tokens):
    """
    Calculate the cost of using GPT-4o-mini API based on the number of tokens.
...
def calculate_openai_cost(model_choice, tokens):
    ...
    if model_choice == 'openai/gpt-3.5-turbo':
        openai_cost = calculate_gpt_pricing(tokens['prompt_tokens'], tokens['completion_tokens'], 
                                 3, 6)
...
def get_answer(query, diet, model_choice, search_type):
    if search_type == 'Vector':
        vector = model.encode(query)
        search_results = elastic_search_knn('question_answer_vector', vector, diet)
    else:
        search_results = elastic_search_text(query, diet)
